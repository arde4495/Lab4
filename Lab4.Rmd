...
ggplot(data = babies, mapping = aes(x = mpregwt, y = `bwt/oz`), color = "green") +
  ...
  ggtitle("Mother's Weight vs Baby's Weight by Smoking Freq.")
...
* I found that there is a minor positive correlation between mothers' pre-pregnancy weights and babys' birth weights with an R value of 0.202.  However, whether or not the mom never smokes or smokes now has no affect on this correlation.
...
```{r, echo =FALSE, error=FALSE, warning=FALSE}
ggplot(data = babies)+
  geom_bar(mapping = aes(x = premature), color = "black", fill = "blue")+
  facet_wrap(~smoke)+
  ggtitle("Count of Premature Births of Smokers vs. Non-Smokers")
```
...
ggplot(data = babies) +
  ...
  facet_wrap(~smoke) + 
  ggtitle("Mother's age for smokers and non-smokers.")
```
